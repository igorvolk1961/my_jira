...
from app.services.assignment_service import covered_for_employee
...
def _mk_project(client):
    ...
    client.post(
        "/employees/create", data={"last_name": "E", "first_name": "E", "position_type_id": "1", "status_id": "1"}
    )
...
def test_task_requires_stage_requirement_and_type(client):
    ...
    client.post(
        "/tasks/create",
        data={"requirement_id": "1", "description": "T", "stage_id": "1", "priority_id": "1", "status_id": "1"},
    )
    # без требования
    client.post(
        "/tasks/create",
        data={"description": "T", "stage_id": "1", "task_type_id": "1", "priority_id": "1", "status_id": "1"},
    )
    # без этапа
    client.post(
        "/tasks/create",
        data={"requirement_id": "1", "description": "T", "task_type_id": "1", "priority_id": "1", "status_id": "1"},
    )
...
def test_one_executor_per_subtask(client):
    ...
    client.post(
        "/subtasks/create", data={"parent_task_id": "1", "description": "S", "priority_id": "1", "status_id": "1"}
    )
    client.post(
        "/employees/create", data={"last_name": "E2", "first_name": "E", "position_type_id": "1", "status_id": "1"}
    )
    client.post("/projects/1/add_employee", data={"employee_id": "2"})
    client.post(
        "/task_assignments/create", data={"task_id": "1", "task_kind": "subtask", "employee_id": "1", "share": "1.0"}
    )
    client.post(
        "/task_assignments/create", data={"task_id": "1", "task_kind": "subtask", "employee_id": "2", "share": "1.0"}
    )
...
def test_employee_load_not_over_100(client):
    ...
    client.post(
        "/task_assignments/create", data={"task_id": "1", "task_kind": "task", "employee_id": "1", "share": "1.0"}
    )
    client.post(
        "/task_assignments/create", data={"task_id": "2", "task_kind": "task", "employee_id": "1", "share": "0.5"}
    )
...
def test_subtask_covers_parent_load(client):
    ...
    session = get_session_factory(main.db_path_for(TEST_DB))()
    rows = (
        session.execute(
            text("SELECT task_id, task_kind, share FROM task_assignment WHERE employee_id=1 AND is_deleted=0")
        )
        .mappings()
        .all()
    )
    covered = covered_for_employee(session, 1)
    session.close()
