...
def test_stakeholder_crud(client):
    client.post(
        "/stakeholders/create", data={"last_name": "Иванов", "first_name": "И", "type_id": "1", "priority": "3"}
    )
    assert "Иванов" in client.get("/stakeholders").get_data(as_text=True)
    client.post(
        "/stakeholders/edit/1", data={"last_name": "Иванов2", "first_name": "И", "type_id": "1", "priority": "3"}
    )
...
def test_project_requirement_stage_task_flow(client):
    ...
    client.post(
        "/requirements/create",
        data={"project_id": "1", "requirement_type_id": "1", "description": "Req", "priority_id": "1"},
    )
...
def test_subtask_edit_delete(client):
    ...
    client.post(
        "/subtasks/edit/1", data={"parent_task_id": "1", "description": "S2", "priority_id": "1", "status_id": "1"}
    )
...
def test_employee_crud(client):
    client.post(
        "/employees/create", data={"last_name": "Сидоров", "first_name": "И", "position_type_id": "1", "status_id": "1"}
    )
    assert "Сидоров" in client.get("/employees").get_data(as_text=True)
    client.post(
        "/employees/edit/1",
        data={"last_name": "Сидоров2", "first_name": "И", "position_type_id": "1", "status_id": "1"},
    )
