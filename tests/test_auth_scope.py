...
def _seed_project_with_user(client):
    ...
    client.post(
        "/stakeholders/create", data={"last_name": "Петров", "first_name": "Иван", "type_id": "1", "priority": "3"}
    )
    ...
    client.post(
        "/task_assignments/create", data={"task_id": "1", "task_kind": "task", "employee_id": str(emp), "share": "1.0"}
    )
    return emp
...
def test_register_requires_all_fields_and_nonempty_password(anon_client):
    anon_client.post(
        "/register", data={"login": "u", "password": "", "last_name": "A", "first_name": "B", "middle_name": "C"}
    )
    assert _scalar("SELECT COUNT(*) FROM app_user WHERE login='u'") == 0
    anon_client.post(
        "/register", data={"login": "u", "password": "p", "last_name": "A", "first_name": "B", "middle_name": ""}
    )
    assert _scalar("SELECT COUNT(*) FROM app_user WHERE login='u'") == 0
    anon_client.post(
        "/register", data={"login": "u", "password": "p", "last_name": "A", "first_name": "B", "middle_name": "C"}
    )
...
def test_user_cannot_do_admin_writes(client):
    ...
    client.post(
        "/employees/create", data={"last_name": "X", "first_name": "Y", "position_type_id": "1", "status_id": "1"}
    )
...
def test_user_can_create_subtask_and_change_status(client):
    ...
    _login_user(client)
    client.post(
        "/subtasks/create", data={"parent_task_id": "1", "description": "Sub", "priority_id": "1", "status_id": "1"}
    )
...
def test_setting_controls_subtask_assignment(client):
    ...
    client.post(
        "/employees/create", data={"last_name": "Второй", "first_name": "С", "position_type_id": "1", "status_id": "1"}
    )
    ...
    client.post(
        "/subtasks/create", data={"parent_task_id": "1", "description": "Sub1", "priority_id": "1", "status_id": "1"}
    )
    client.post(
        "/subtasks/create", data={"parent_task_id": "1", "description": "Sub2", "priority_id": "1", "status_id": "1"}
    )
    _login_user(client)
    # по умолчанию ON — можно назначить другого
    client.post(
        "/task_assignments/create",
        data={"task_id": "1", "task_kind": "subtask", "employee_id": str(other), "share": "1.0"},
    )
    assert (
        _scalar("SELECT employee_id FROM task_assignment WHERE task_kind='subtask' AND task_id=1 AND is_deleted=0")
        == other
    )
    ...
    client.post(
        "/task_assignments/create",
        data={"task_id": "2", "task_kind": "subtask", "employee_id": str(other), "share": "1.0"},
    )
    assert _scalar("SELECT COUNT(*) FROM task_assignment WHERE task_kind='subtask' AND task_id=2 AND is_deleted=0") == 0
    # себя — можно
    client.post(
        "/task_assignments/create",
        data={"task_id": "2", "task_kind": "subtask", "employee_id": str(emp), "share": "1.0"},
    )
    assert (
        _scalar("SELECT employee_id FROM task_assignment WHERE task_kind='subtask' AND task_id=2 AND is_deleted=0")
        == emp
    )
    # «не назначен» — снятие
    client.post(
        "/task_assignments/create", data={"task_id": "2", "task_kind": "subtask", "employee_id": "", "share": "1.0"}
    )
    assert _scalar("SELECT COUNT(*) FROM task_assignment WHERE task_kind='subtask' AND task_id=2 AND is_deleted=0") == 0
...
def test_audit_logged_and_restricted(client):
    ...
    client.post(
        "/register", data={"login": "user1", "password": "pw", "last_name": "И", "first_name": "И", "middle_name": "И"}
    )
    _login_user(client)
...
def test_owner_cannot_manage_subtask_assigned_to_other(client):
    ...
    client.post(
        "/subtasks/create",
        data={"parent_task_id": "1", "description": "Owned", "priority_id": "1", "status_id": str(_status_id("Новая"))},
    )
    client.post(
        "/task_assignments/create",
        data={"task_id": "1", "task_kind": "subtask", "employee_id": str(other), "share": "1.0"},
...
def test_employee_delete_allows_others_but_not_self(client):
    ...
    client.post(
        "/register", data={"login": "u2", "password": "p", "last_name": "У", "first_name": "У", "middle_name": "У"}
    )
...
def test_position_and_analyst_role_sync(client):
    ...
    client.post(
        ...
        data={"last_name": "Иванов", "first_name": "Иван", "position_type_id": str(analyst), "status_id": "1"},
    )
...
def test_employee_role_change_forbidden_for_user(client):
    _seed_project_with_user(client)
    client.post(
        "/register", data={"login": "u3", "password": "p", "last_name": "Т", "first_name": "Т", "middle_name": "Т"}
    )
...
def test_database_copy_preserves_data(client):
    ...
    try:
        r = client.post("/database/copy", data={"source": main.DEFAULT_DB_NAME, "name": "pytest_copy", "switch": "1"})
        assert r.status_code == 302
