...
## 12.9 Шаблоны интервью

- В разделе «Интервью» — кнопка **«Шаблоны интервью»**.
- Форма нового интервью: выбор стейкхолдера, **шаблон вопросов** («— с нуля —» или один из шаблонов) и дата-время.
- Карточка интервью: кнопка **«Сохранить как шаблон»** (создать шаблон из вопросов интервью).
- Карточка шаблона: вопросы с добавлением/правкой/удалением и кнопка **«Создать интервью по шаблону»**.

```mermaid
flowchart LR
    Interviews[Интервью] --> TplList[Шаблоны интервью]
    TplList --> TplCard[Карточка шаблона]
    TplCard --> NewIv[Новое интервью по шаблону]
    IvCard[Карточка интервью] --> SaveTpl[Сохранить как шаблон]
```
